...
(function (global) {
  ...
  /* ============================================================
     LEVELY
     name      – jméno sektoru
     arena     – interiér místnosti s bossem: když do ní hráč vejde, dveře
                 se zapečetí a boss se teprve objeví (null = bez arény)
     ============================================================ */
  var LEVELS = [
    {
      name: 'Pevnost',
      bossName: 'Generál Wolfheim', bossHp: 480, needBoss: true,
      arena: { x0: 20, y0: 31, x1: 37, y1: 38 },
      grid: [
        /* 00 */ '########################################',
        /* 01 */ '####################===================#',
        /* 02 */ '##............######=.................=#',
    ...
    },
    {
      ...
      ['ammo', 4, 2], ['shells', 2, 4],
      /* --- strážnice --- */
      ['guard', 10, 2], ['guard', 13, 4], ['table', 11, 3], ['medkit', 9, 5], ['ammo', 13, 1], ['cross', 9, 1],
      /* --- předsíň --- */
      ['guard', 18, 2], ['dog', 21, 5], ['barrel', 17, 1], ['shells', 19, 5], ['crown', 21, 1],
      /* --- pokladnice --- */
      ['guard', 27, 1], ['cross', 25, 1], ['cross', 29, 1], ['chalice', 27, 3], ['crown', 25, 5], ['ammoBig', 29, 5],
      /* --- stříbrná komora --- */
      ['keySilver', 19, 11], ['officer', 17, 9], ['officer', 21, 13], ['guard', 22, 8], ['guard', 16, 14],
      ['pillar', 17, 13], ['pillar', 21, 9], ['brazier', 16, 8], ['brazier', 22, 14], ['medkit', 16, 11], ['ammo', 22, 11],
      /* --- kasárna --- */
      ['guard', 9, 9], ['guard', 13, 9], ['guard', 9, 13], ['guard', 13, 13], ['officer', 11, 9],
      ['table', 10, 11], ['table', 11, 11], ['table', 12, 11], ['medkit', 8, 14], ['ammo', 14, 8], ['food', 14, 14], ['bones', 8, 8],
      /* --- tajný výklenek --- */
      ['cross', 4, 10], ['crown', 6, 10], ['chalice', 4, 12], ['shells', 6, 12], ['medkit', 5, 10], ['ammoBig', 5, 12],
      /* --- psinec --- */
      ['dog', 9, 17], ['dog', 13, 17], ['dog', 9, 21], ['dog', 13, 21],
      ['barrel', 8, 16], ['barrel', 14, 22], ['bones', 8, 22], ['bones', 14, 16], ['medkit', 11, 19], ['ammo', 8, 19],
      /* --- zlatá komora --- */
      ['keyGold', 19, 19], ['officer', 17, 17], ['officer', 21, 21], ['officer', 21, 17], ['guard', 17, 21],
      ['pillar', 17, 19], ['pillar', 21, 19], ['brazier', 16, 16], ['brazier', 22, 22], ['medkit', 22, 16], ['ammoBig', 16, 22], ['chalice', 19, 16],
      /* --- zbrojnice --- */
      ['shotgun', 27, 11], ['shells', 25, 9], ['shells', 29, 9], ['shells', 27, 13], ['ammoBig', 25, 13], ['ammoBig', 29, 13],
      ['guard', 24, 8], ['guard', 30, 14], ['officer', 30, 8], ['barrel', 24, 14], ['barrel', 25, 14], ['table', 27, 9],
      /* --- výtah --- */
      ['officer', 25, 17], ['officer', 29, 21], ['officer', 29, 17], ['guard', 25, 21],
      ['pillar', 26, 18], ['pillar', 28, 18], ['pillar', 26, 20], ['pillar', 28, 20],
      ['brazier', 24, 16], ['brazier', 30, 22], ['medkit', 24, 22], ['ammoBig', 30, 16], ['crown', 27, 17]
      ],
      secrets: [{ x: 7, y: 11, dx: -1, dy: 0, tiles: 2 }],
      rooms: [
        { name: 'Kasematy',        x0: 0, y0: 0, x1: 39, y1: 39, col: '#4a4a52' },
        { name: 'Cela',            x0: 1, y0: 1, x1: 5, y1: 5, col: '#4f4f58' },
        { name: 'Strážnice',       x0: 8, y0: 0, x1: 14, y1: 6, col: '#7a5a34' },
        { name: 'Předsíň',         x0: 16, y0: 0, x1: 22, y1: 6, col: '#5a4a3a' },
        { name: 'Pokladnice',      x0: 24, y0: 0, x1: 30, y1: 6, col: '#8a6a20' },
        { name: 'Kasárna',         x0: 7, y0: 7, x1: 15, y1: 15, col: '#7a5a34' },
        { name: 'Stříbrná komora', x0: 15, y0: 7, x1: 23, y1: 15, col: '#6d4046' },
        { name: 'Zbrojnice',       x0: 23, y0: 7, x1: 31, y1: 15, col: '#4e5a6d' },
        { name: 'Tajný výklenek',  x0: 3, y0: 9, x1: 7, y1: 13, col: '#8a6a20' },
        { name: 'Psinec',          x0: 7, y0: 15, x1: 15, y1: 23, col: '#6a6a74' },
        { name: 'Zlatá komora',    x0: 15, y0: 15, x1: 23, y1: 23, col: '#7a5a34' },
        { name: 'Výtah',           x0: 23, y0: 15, x1: 31, y1: 23, col: '#3d2a4e' }
      ],
      start: { x: 3.5, y: 3.5, a: 0 }
    },
    {
      ...
      /* --- aréna: boss se objeví, až se za hráčem zavřou dveře --- */
  ...
  var Level = {
    ...
    start: null,
    bossName: null, bossHp: 0, needBoss: false,
    arena: null,         // interiér arény s bossem (viz LEVELS)
    switchOn: false,
    ...
    for (y = 0; y < MH; y++) {
      for (x = 0; x < MW; x++) {
        ...
        var d = {
          ...
          lock: v === 8 ? 1 : (v === 9 ? 2 : 0),   // 0 volné, 1 zlatý, 2 stříbrný, 3 zapečetěné
          lock0: v === 8 ? 1 : (v === 9 ? 2 : 0),  // původní zámek (pro odpečetění)
          sealed: false,
          tex: ID2TEX[v],
